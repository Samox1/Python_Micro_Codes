...
Siec <- trainNN( Xiris, Y_tar, h = 5, lr =  0.01, Maxiter = 100 )
Siec
...
wprzod <- function( X, list( 1= Win, 2 = Wout ) ){
  n <- length( list() )
  for( i in 1:n ){
    H1 <- sigmoid( cbind( 1, X ) %*% list[[i]] )
    H2 <- sigmoid( cbind( 1, H1 ) %*% list[[i+1]] )
  }
  x1 <- x2 <- X
  w1 <- w2 <- matrix( runif(n*n), n, n  )
 # x1 %*% w1
  z1<-eval( parse( text = paste0( "x", 1, "%*%", "w", 1 ) ) )
  #sprintf("x_%d",1)
  L <- list(w1,w2)
  n1 <- length(  list(w1,w2) )
  for(j in 1:n1){
 z2<- eval( parse( 
    text = sprintf("H1 <-sigmoid( x%d %%*%% L[[%d]])", j, j) ) )}
  # z1 <- cbind( 1, X )  %*% Win
  # H <- sigmoid( z1 )
  # z2 <- cbind( 1, H )  %*% Wout
  # y_hat <- sigmoid( z2 )
  return( list( y_hat = y_hat, H =list(H1,H2) )  )
}
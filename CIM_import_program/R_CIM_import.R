...
Input <- read_lines("D:/Programming/Python_Micro_Codes/CIM_import_program/MIKRONIKA_23A10h_Original.xml", skip_empty_rows = FALSE)
...
# XML_Lista_ID <- read_xml(Input,as_html = FALSE)
print("--- read_xml ---")
...
for (y in c(4:length(XML_Tabela))) {
  kappa1 <- which(grepl("^$", XML_Tabela[,y]))
  if (length(kappa1) > 0){
    for (x in kappa1) {
    numeros <- which(grepl(colnames(XML_Tabela[y]), names(XML_Listowa$RDF[[x]])))
    XML_Tabela[x,y] <- xml_attrs(xml_child(xml_child(XML_Lista_ID, x), numeros))
    }
  }
}
...
write_excel_csv(XML_Tabela, "D:/Programming/Python_Micro_Codes/CIM_import_program/MIKRONIKA_ALL_Output.csv")
